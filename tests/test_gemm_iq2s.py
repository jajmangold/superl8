...
"""Fused GGUF IQ2_S dp4a GEMV (superl8#317).

Gate the kernel against the fp32 reference dequant of the SAME bytes
(`superl8.quant.iq2s`, itself bit-exact vs gguf-py). Every 82-byte pattern is a valid
IQ2_S block, so random bytes cover a 10-bit index (8 bits qs + 2 of qh) into a 1024-entry grid, explicit per-weight
sign bits, and a 4-bit scale per SIXTEEN weights
...
import superl8
from superl8.quant.iq2s import BLOCK_BYTES, QK_K, dequantize_iq2_s_tensor
...
def _weights(n_out: int, in_features: int, seed: int = 0):
    """Random native IQ2_S bytes + their fp32 dequant."""
    ...
    return w, dequantize_iq2_s_tensor(w, in_features)
...
def test_decode_matches_the_reference(M):
    ...
    got = superl8.linear_iq2s(x, w).float()
...
def test_tile_matches_the_reference(M, N, K):
    ...
    got = superl8.linear_iq2s(x, torch.from_numpy(w_np).cuda()).float()
    want = x.float() @ torch.from_numpy(w_ref).cuda().t()
...
def test_tile_and_decode_agree_on_the_same_row():
    ...
    decode = superl8.linear_iq2s(x[:1], w).float()      # M=1  -> MMVQ
    tile = superl8.linear_iq2s(x, w).float()[:1]        # M=24 -> tile
...
def test_decode_is_deterministic():
    ...
    a = superl8.linear_iq2s(x, w)
    for _ in range(2):
        assert torch.equal(superl8.linear_iq2s(x, w), a)
...
def test_bias_is_applied():
    ...
    base = superl8.linear_iq2s(x, w).float()
    with_bias = superl8.linear_iq2s(x, w, bias=bias).float()
