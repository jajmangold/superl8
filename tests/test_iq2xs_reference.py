...
"""IQ2_XS reference dequant, gated against gguf-py (ggml's own implementation).

Every 74-byte pattern is a valid IQ2_XS block, so random bytes exercise the grid/codebook,
the sign bits and the scales without a model file (superl8#317).
"""
import numpy as np
import pytest

from superl8.quant.iq2xs import BLOCK_BYTES, QK_K, dequantize_iq2_xs, dequantize_iq2_xs_tensor
...
def _oracle(blocks: np.ndarray) -> np.ndarray:
    ...
    out = dequantize(np.ascontiguousarray(blocks, dtype=np.uint8), GGMLQuantizationType.IQ2_XS)
...
def test_matches_gguf_py_on_random_blocks(seed):
    b = _blocks(31, seed)
    np.testing.assert_array_equal(dequantize_iq2_xs(b), _oracle(b).reshape(31, QK_K))


def test_zero_block_is_all_zero():
    assert not dequantize_iq2_xs(np.zeros((2, BLOCK_BYTES), dtype=np.uint8)).any()


def test_tensor_round_trip():
    in_features, n_out = 512, 3
    w = _blocks(n_out * (in_features // QK_K), 7).reshape(n_out, -1)
    out = dequantize_iq2_xs_tensor(w, in_features)
...
def test_block_geometry_is_the_ggml_one():
    ...
    assert GGML_QUANT_SIZES[GGMLQuantizationType.IQ2_XS] == (QK_K, BLOCK_BYTES)
    assert BLOCK_BYTES == 74
