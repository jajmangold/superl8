...
"""Fused GGUF IQ2_XS dp4a GEMV (superl8#317).

Gate the kernel against the fp32 reference dequant of the SAME bytes
(`superl8.quant.iq2xs`, itself bit-exact vs gguf-py). Every 74-byte pattern is a valid
IQ2_XS block, so random bytes cover a 9-bit grid index plus a 7-bit ksigns_iq2xs index per uint16, and 4-bit scales at
16-weight granularity
...
import superl8
from superl8.quant.iq2xs import BLOCK_BYTES, QK_K, dequantize_iq2_xs_tensor
...
def _weights(n_out: int, in_features: int, seed: int = 0):
    """Random native IQ2_XS bytes + their fp32 dequant."""
    ...
    return w, dequantize_iq2_xs_tensor(w, in_features)
...
def test_decode_matches_the_reference(M):
    ...
    got = superl8.linear_iq2xs(x, w).float()
...
def test_tile_matches_the_reference(M, N, K):
    ...
    got = superl8.linear_iq2xs(x, torch.from_numpy(w_np).cuda()).float()
    want = x.float() @ torch.from_numpy(w_ref).cuda().t()
...
def test_tile_and_decode_agree_on_the_same_row():
    ...
    decode = superl8.linear_iq2xs(x[:1], w).float()      # M=1  -> MMVQ
    tile = superl8.linear_iq2xs(x, w).float()[:1]        # M=24 -> tile
...
def test_decode_is_deterministic():
    ...
    a = superl8.linear_iq2xs(x, w)
    for _ in range(2):
        assert torch.equal(superl8.linear_iq2xs(x, w), a)
...
def test_bias_is_applied():
    ...
    base = superl8.linear_iq2xs(x, w).float()
    with_bias = superl8.linear_iq2xs(x, w, bias=bias).float()
