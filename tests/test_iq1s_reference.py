...
"""IQ1_S reference dequant, gated against gguf-py (ggml's own implementation).

Every 50-byte pattern is a valid IQ1_S block, so random bytes exercise the grid/codebook,
the sign bits and the scales without a model file (superl8#317).
"""
import numpy as np
import pytest

from superl8.quant.iq1s import BLOCK_BYTES, QK_K, dequantize_iq1_s, dequantize_iq1_s_tensor
...
def _oracle(blocks: np.ndarray) -> np.ndarray:
    ...
    out = dequantize(np.ascontiguousarray(blocks, dtype=np.uint8), GGMLQuantizationType.IQ1_S)
...
def test_matches_gguf_py_on_random_blocks(seed):
    b = _blocks(31, seed)
    np.testing.assert_array_equal(dequantize_iq1_s(b), _oracle(b).reshape(31, QK_K))


def test_zero_block_is_all_zero():
    assert not dequantize_iq1_s(np.zeros((2, BLOCK_BYTES), dtype=np.uint8)).any()


def test_tensor_round_trip():
    in_features, n_out = 512, 3
    w = _blocks(n_out * (in_features // QK_K), 7).reshape(n_out, -1)
    out = dequantize_iq1_s_tensor(w, in_features)
...
def test_block_geometry_is_the_ggml_one():
    ...
    assert GGML_QUANT_SIZES[GGMLQuantizationType.IQ1_S] == (QK_K, BLOCK_BYTES)
    assert BLOCK_BYTES == 50
