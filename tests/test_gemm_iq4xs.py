...
"""Fused GGUF IQ4_XS dp4a GEMV (superl8#317 work item 2c).

Gate the kernel against the fp32 reference dequant of the SAME bytes (`superl8.quant.iq4xs`,
itself bit-exact vs gguf-py). Random 136-byte blocks cover the codebook, both nibble
...
import superl8
from superl8.quant.iq4xs import BLOCK_BYTES, QK_K, dequantize_iq4_xs_tensor
...
def _weights(n_out: int, in_features: int, seed: int = 0):
    ...
    return w, dequantize_iq4_xs_tensor(w, in_features)
...
def test_decode_matches_the_reference(M):
    ...
    got = superl8.linear_iq4xs(x, torch.from_numpy(w_np).cuda()).float()
...
def test_nibble_halves_are_not_swapped():
    """A swapped low/high unpack still correlates; compare against the oracle elementwise
    on a single row so an interleave bug cannot hide behind a good cosine."""
    torch.manual_seed(5)
    N, K = 64, 256
    ...
    x[0, 16] = 2.0       # weight 16 — the HIGH nibble of qs[0]
    got = superl8.linear_iq4xs(x, torch.from_numpy(w_np).cuda()).float()
    want = x.float() @ torch.from_numpy(w_ref).cuda().t()
...
def test_tile_and_decode_agree_on_the_same_row():
    ...
    decode = superl8.linear_iq4xs(x[:1], w).float()      # M=1  -> MMVQ
    tile = superl8.linear_iq4xs(x, w).float()[:1]        # M=24 -> tile
...
def test_decode_is_deterministic():
    ...
    a = superl8.linear_iq4xs(x, w)
    for _ in range(2):
        assert torch.equal(superl8.linear_iq4xs(x, w), a)
