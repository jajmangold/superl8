...
"""Fused GGUF IQ3_S dp4a GEMV (superl8#317 work item 2).

Gate the kernel against the fp32 reference dequant of the SAME bytes (`superl8.quant.iq3s`,
itself bit-exact vs gguf-py). Every 110-byte pattern is a valid IQ3_S block, so random
bytes cover the grid, the sign bits and the per-32 scales without a model file.
"""
import numpy as np
import pytest
import torch
...
import superl8
from superl8.quant.iq3s import BLOCK_BYTES, QK_K, dequantize_iq3_s_tensor
...
def _weights(n_out: int, in_features: int, seed: int = 0):
    """Random native IQ3_S bytes + their fp32 dequant."""
    rng = np.random.default_rng(seed)
    nsb = in_features // QK_K
    w = rng.integers(0, 256, size=(n_out, nsb * BLOCK_BYTES), dtype=np.uint8)
    # fp16 super-block scales: keep them modest and finite (bytes 0..1 of each block)
    ...
    return w, dequantize_iq3_s_tensor(w, in_features)
...
def test_decode_matches_the_reference(M):
    ...
    got = superl8.linear_iq3s(x, w).float()
...
def test_tile_matches_the_reference(M, N, K):
    ...
    got = superl8.linear_iq3s(x, torch.from_numpy(w_np).cuda()).float()
    want = x.float() @ torch.from_numpy(w_ref).cuda().t()
...
def test_tile_and_decode_agree_on_the_same_row():
    ...
    decode = superl8.linear_iq3s(x[:1], w).float()      # M=1  -> MMVQ
    tile = superl8.linear_iq3s(x, w).float()[:1]        # M=24 -> tile
...
def test_decode_is_deterministic():
    ...
    a = superl8.linear_iq3s(x, w)
    for _ in range(2):
        assert torch.equal(superl8.linear_iq3s(x, w), a)
...
def test_bias_is_applied():
    ...
    base = superl8.linear_iq3s(x, w).float()
    with_bias = superl8.linear_iq3s(x, w, bias=bias).float()
